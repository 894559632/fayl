...
@Controller
@RequestMapping(value = "/Verwalter/company")
public class TdManagerCompanyController {
	...
	@RequestMapping
	public String company(Long status, ModelMap map, HttpServletRequest req) {
		String username = (String) req.getSession().getAttribute("manager");
		if (null == username) {
			return "redirect:/Verwalter/login";
		}

		map.addAttribute("setting", TdCompany.getInstance());
		map.addAttribute("status", status);

		// 查询所有的公司信息
		map.addAttribute("info_list", tdCompanyinfoService.findAll());

		return "/site_mag/company_edit";
	}

	@RequestMapping(value = "/save")
	public String orderEdit(TdCompany TdCompany, ModelMap map, HttpServletRequest req) {
		String username = (String) req.getSession().getAttribute("manager");
		if (null == username) {
			return "redirect:/Verwalter/login";
		}
		...
		if (null == TdCompany.getId()) {
			tdManagerLogService.addLog("add", "用户修改公司信息", req);
		} else {
			tdManagerLogService.addLog("edit", "用户修改公司信息", req);
		}

		TdCompanyService.save(TdCompany);
		tdCommonService.initCompany();

		return "redirect:/Verwalter/company?status=1";
	}
	...
	//
	// return "/company/suggestion_list";
	// }
	//
	//
	// ///////////////////////////////////
	...
	public String serviceItemEdit(TdServiceItem tdServiceItem, String __VIEWSTATE, ModelMap map,
		...
		return "redirect:/Verwalter/company/service/list";
	}
	...
	public void getModel(@RequestParam(value = "id", required = false) Long id,
		...
		if (null != id) {
			map.addAttribute("TdCompany", TdCompanyService.findOne(id));
		}
