...
@Controller
@RequestMapping(value = "/Verwalter/company/info")
public class TdManagerCompanyInfoController {
	...
	public String goodsListPost(Integer page, Integer size, String saleType, String property, String __EVENTTARGET,
		...
		if (null != __EVENTTARGET) {
			...
			case "btnSave":
				...
				tdManagerLogService.addLog("edit", "用户修改公司信息列表项", req);
				break;

			case "btnDelete":
				btnDelete(listId, listChkId);
				tdManagerLogService.addLog("delete", "用户删除公司信息列表项", req);
				break;
		...
		if (null != keywords && !"".equals(keywords)) {
			Page<TdCompanyInfo> goodsPage = tdCompanyInfoService.findByTitleContainingOrderBySortIdAsc(keywords, page,
					size);
			map.addAttribute("content_page", goodsPage);
		} else {
			Page<TdCompanyInfo> goodsPage = tdCompanyInfoService.findAll(page, size);
			map.addAttribute("content_page", goodsPage);
		}
		...
		return "/site_mag/company_info_list";
	...
	public String goodsEdit(Long id, String __EVENTTARGET, String __EVENTARGUMENT, String __VIEWSTATE, ModelMap map,
		...
		return "/site_mag/company_info_edit";
	}

	@RequestMapping(value = "/save", method = RequestMethod.POST)
	public String save(TdCompanyInfo info, String[] hid_photo_name_show360, String __EVENTTARGET,
			String __EVENTARGUMENT, String __VIEWSTATE, String menuId, String channelId, ModelMap map,
			Boolean isIndexRecommend, Boolean isRecommendType, Boolean isHot, Boolean isNew, Boolean isSpecialPrice,
			HttpServletRequest req) {
		String username = (String) req.getSession().getAttribute("manager");
		if (null == username) {
			return "redirect:/Verwalter/login";
		...
		if (null != info && (null == info.getNumber() || "".equalsIgnoreCase(info.getNumber()))) {
			...
			String number = "GSXX" + dateFormat + randomNumber;
			info.setNumber(number);
		}
		info = tdCompanyInfoService.save(info);
		tdManagerLogService.addLog(type, "用户修改公司信息项", req);

		map.addAttribute("goods", info);

		return "redirect:/Verwalter/company/info/list?__EVENTTARGET=" + __EVENTTARGET + "&__EVENTARGUMENT="
	...
	private void btnSave(Long[] ids, Double[] sortIds, String username) {
		...
		for (int i = 0; i < ids.length; i++) {
			...
			TdCompanyInfo goods = tdCompanyInfoService.findOne(id);

			if (sortIds.length > i) {
				goods.setSortId(sortIds[i]);
				tdCompanyInfoService.save(goods);
			}
		}
	}
	...
	private void btnDelete(Long[] ids, Integer[] chkIds) {
		...
		for (int chkId : chkIds) {
			if (chkId >= 0 && ids.length > chkId) {
				...
				tdCompanyInfoService.delete(id);
			}
		}
	}
